...
void mazeDimension(FILE* fp, int* a_rows, int* a_cols)
{
	rewind(fp);
	char ch = '0';
	bool first_row = true;
...
char getLocation(FILE* fp, int row, int col)
{
	rewind(fp);
	char ch = '0';
	int i = 0;
...
Maze* readMaze(FILE* fp)
{
	...
	for(int i = 0; i < maze -> n_row; i++)
	{
		...
		{
			maze -> maze_array[i][j] = getLocation(fp,i,j);
		}
	}
...
bool writeMaze(const char* filename, const Maze* maze)
{
...
void findDist(Maze *maze, int curr, int curc, int endr, int endc, int count,  int** dists)
{
	maze->maze_array[curr][curc] = VISITED;
	...
	if (_helper_distances(maze, curr + 1, curc, dists, count + 1))
	{
		...
		findDist(maze, curr + 1, curc, endr, endc, count + 1, dists);
	}
	if (_helper_distances(maze, curr - 1, curc, dists, count + 1))
	{
		if(dists[curr - 1][curc] == -1 || dists[curr - 1][curc] > count + 1)
	...
	if (_helper_distances(maze, curr, curc + 1, dists, count + 1))
	{
		...
		findDist(maze, curr, curc + 1, endr, endc, count + 1, dists);
	}
	if (_helper_distances(maze, curr, curc - 1, dists, count + 1))
	{
		if(dists[curr][curc - 1] == -1 || dists[curr][curc - 1] > count + 1)
...
bool wallHit(Maze* maze, int** dists, int row, int col)
{
	bool wall = false;
   if ((row >= 0) && (row < maze->n_row))
   {
...
void _shortest_path(Maze* maze, int** dists, int endr, int endc, char* directions, int count)
{
	...
	int short_path[4];
	if(!wallHit(maze, dists, endr + 1, endc))
	{
	...
	if(!wallHit(maze, dists, endr, endc + 1))
	{
                        ...
                        Coord source, Coord destination)
{
	...
	findDist(maze, source.row, source.col, destination.row, destination.col, 0, distances); // creates 2d array paths = distance from source walls = -1
